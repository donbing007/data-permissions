 ...
 */
public class JSqlParserConditionHandlerTest {
    ...
    public void testAddCondition() throws Exception {
        ...
        JSqlParserConditionHandler handler;
        AddConditionPack cp;
        Statement statement;
        for (String sql : sqls) {
            statement = CCJSqlParserUtil.parse(sql);
            handler = new JSqlParserConditionHandler(statement);
            cp = data.get(sql);
    ...
    private Map<String, RemoveConditionPack> buildRemoveCase() throws Exception {
        ...
        data.put("select * from t1 where c1=10 and c2='10' or c3 is null and c4=c1+1",
            new RemoveConditionPack(
                new Condition(new Field("c4"), ConditionOperator.EQUALS, new ArithmeticValue("c1+1")),
    ...
    public void testIterCondition() throws Exception {
        ...
        JSqlParserConditionHandler handler;
        for (String sql : sqls) {
            handler = new JSqlParserConditionHandler(CCJSqlParserUtil.parse(sql));
    ...
    private Map<String, List<Condition>> buildIterCase() throws Exception {
        ...
        data.put("select * from t1 where a=a+1",
            Arrays.asList(
                new Condition(new Field("a"), ConditionOperator.EQUALS, new ArithmeticValue("a+1"))
            ));
